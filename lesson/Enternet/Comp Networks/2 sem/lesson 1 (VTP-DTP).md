...
VTP - позволяет автоматизировать настройку VLAN на новое оборудование
...
Если на S1(сервере) появляется новый VLAN, то коммутатор отправляет сообщение VTP Summary Advertisment в котором
сообщается номер ревизии + 1 (В Summary еще нет никаких изменений только номер ревизии)

Номер ревизии - номер версии пересмотра конфигурации VLAN

Далее клиент сравнивает свой номер ревизии с пришедшим ему в сообщении, если он больше, то клиент кидает VTP Request и
запрашивает изменения

Сервер опять кидает Advertisment с настройками, клиент увеличивает номер ревизии на 1

Теперь у всех одинаковый номер ревизии и актуальные данные

**!Важно** клиент будет игнорить такие сообщения если они с другого домена. 
...
DTP - позволяет автоматически согласовывать режим порта(транка) который у нас подключается к другому оборудованию
